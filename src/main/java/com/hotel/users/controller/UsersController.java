...
public class UsersController {
	...
	/*
	 * @PostMapping("/login") public ResponseEntity<SaveUserResponse>
	 * LoginUser(@Valid @RequestBody LoginRequest loginReq) { //UserBean user =
	 * req.getUserBean(); SaveUserResponse response= new SaveUserResponse(); UserDTO
	 * userto = userService.saveUser(user); BeanUtils.copyProperties(userto, user);
	 * response.setUserBean(user); return new
	 * ResponseEntity<SaveUserResponse>(response, HttpStatus.OK); }
	 */
}
